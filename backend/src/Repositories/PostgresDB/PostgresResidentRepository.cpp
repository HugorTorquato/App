...
int PostgresResidentRepository::save(const Resident& resident) {
    ...
    Logger::info(__FUNCTION__ + std::string("Saving resident to PostgreSQL database: " + resident.getFullName()));
...
std::optional<Resident> PostgresResidentRepository::findById(int id) {
    ...
    Logger::info(__FUNCTION__ + std::string("Finding resident by ID in PostgreSQL database: " + std::to_string(id)));
...
std::vector<Resident> PostgresResidentRepository::findAll() {
    ...
    Logger::info(__FUNCTION__ + std::string("Finding all residents in PostgreSQL database"));
...
void PostgresResidentRepository::update(const Resident& resident) {
    ...
    Logger::info(__FUNCTION__ + std::string("Updating resident in PostgreSQL database: " + resident.getFullName()));
}

void PostgresResidentRepository::remove(int id) {
    // Implementation for removing a resident from PostgreSQL database
    Logger::info(__FUNCTION__ + std::string("Removing resident from PostgreSQL database with ID: ") + std::to_string(id));
}
